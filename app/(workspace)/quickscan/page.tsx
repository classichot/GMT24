    ...
    <div style={{ display: "grid", gap: 20 }}>
      ...
      <ScanRunner
        ...
        onUpload={async (file, period) => {
          ...
          const r = ai.runScan(res.resolved?.name ?? file.name.replace(/\.[a-z]+$/i, ""), { attachmentId: a.id, period, registryId: res.resolved?.registryId });
          setCurrent(r.id);
        }}
      />
      ...
      {!scan && (
        <div className="panel"><div className="panel-body text-muted" style={{ fontSize: 13 }}>
          No scan yet. Enter a company name above (Thai or English — a subsidiary name resolves to its parent), or upload an annual report. The registry in this build is the GMT24 demonstration corpus; real groups are scanned from uploaded disclosures through the same pipeline.
        </div></div>
      )}
    </div>
  );
