...
export default function QualityPage() {
  ...
  const { t } = useCalc();
  const mapped = DATA.accounts.filter((a) => a.approved || approvedMaps[a.account]).length;
    ...
    <div>
      <div className="grid-score" style={{ marginBottom: 24 }}>
        <div className="panel" style={{ padding: 24, textAlign: "center" }}>
          ...
          <p className="text-muted">{DATA.issues.filter((i) => i.severity === "block").length} issues must be resolved before lock. {mapped}/{DATA.accounts.length} sample accounts mapped.</p>
        </div>
        <div className="panel">
