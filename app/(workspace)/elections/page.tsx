...
export default function ElectionsPage() {
  ...
  const liveTh = applyPackage(calcs, { stock: ["TH"] }).find((r) => r.iso === "TH");
  ...
  return (
    <div>
      ...
      <div className="callout" style={{ marginBottom: 20, display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        <div>
          <strong>GMT24 Election & Scenario Engine.</strong> Pillar Two is not one deterministic calculation. The engine posts the Core baseline, then detects every OECD-permitted election and safe harbour at the correct scope — group, jurisdiction, CE, transaction, asset class, or DTL item. A jurisdiction election binds every Constituent Entity in that country. The LLM does not flip an election.
        </div>
        <div className="stack-actions">
          <Link href="/optimize" className="btn btn-primary">Optimize my GloBE position</Link>
          <Link href="/playbook/elections" className="btn btn-secondary">Playbook</Link>
      ...
      <div className="kpi-grid cols-4" style={{ marginBottom: 20 }}>
        <div className="kpi">
          <div className="kpi-label">Register</div>
          <div className="kpi-val">{O.counts.total}</div>
          <div className="kpi-sub">{O.counts.fiveYear} five-year locks · GIR section D</div>
        </div>
        <div className="kpi">
          <div className="kpi-label">Available on this snapshot</div>
          <div className="kpi-val">{O.counts.available}</div>
          <div className="kpi-sub">{O.counts.review} review · {O.counts.unavailable} unavailable</div>
        </div>
        <div className="kpi">
          <div className="kpi-label">Thai stock-comp (live)</div>
          <div className="kpi-val">{liveTh ? pct(liveTh.etr, 1) : "—"}</div>
          <div className="kpi-sub">
            Core {pct(O.thBase.etr, 1)} · top-up {liveTh ? <Amount n={liveTh.topUp} audit={O.audit} compact /> : "—"}
          </div>
        </div>
        <div className="kpi">
          <div className="kpi-label">Scope rule</div>
          <div className="kpi-val" style={{ fontSize: 16, lineHeight: 1.3 }}>All CEs in the jurisdiction</div>
          <div className="kpi-sub">Art. 3.2.2 binds TH001 and Rayong PE together</div>
        </div>
      </div>
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        <div className="panel-head">
          ...
          <span className="tag tag-accent">Jurisdiction / 5-year</span>
        </div>
        <div className="panel-body">
          ...
          <div className="kpi-grid cols-4" style={{ marginTop: 16 }}>
            ...
            <div className="kpi">
              <div className="kpi-label">Live overlay (USD)</div>
              <div className="kpi-val">{liveTh ? pct(liveTh.etr, 1) : "—"}</div>
              <div className="kpi-sub">
                GloBE {liveTh ? eur(liveTh.globe, true) : "—"} vs Core {eur(O.thBase.globeIncome, true)}
              </div>
            </div>
            <div className="kpi">
              <div className="kpi-label">Bound entities</div>
              <div className="kpi-val" style={{ fontSize: 16 }}>TH001 · TH-PE1</div>
              <div className="kpi-sub">Not an entity-by-entity switch</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="panel" style={{ marginBottom: 20 }}>
        <div className="panel-head">
          <h4>Election register · eligibility engine</h4>
          <span className="tag tag-neutral">{rows.length} shown</span>
        </div>
        <div className="table-wrap">
          <table className="table">
            <thead>
