...
${j.ceByCe.rows.map((r) => `        <gir26:CE code="${esc(r.code)}"><gir26:Name>${esc(r.name)}</gir26:Name><gir26:FANIL currency="USD">${amount(r.fanil)}</gir26:FANIL><gir26:Adjustments currency="USD" count="${r.adjustmentCount}">${amount(r.adjustments)}</gir26:Adjustments><gir26:CurrentTax currency="USD">${amount(r.currentTax)}</gir26:CurrentTax><gir26:DeferredTax currency="USD">${amount(r.deferredTax)}</gir26:DeferredTax><gir26:Payroll currency="USD">${amount(r.payroll)}</gir26:Payroll><gir26:TangibleAssets currency="USD">${amount(r.tangible)}</gir26:TangibleAssets><gir26:GloBEIncome currency="USD">${amount(r.globeIncome)}</gir26:GloBEIncome><gir26:DeemedGloBEIncome currency="USD">${amount(r.deemedGlobe)}</gir26:DeemedGloBEIncome><gir26:Share>${(r.share * 100).toFixed(4)}</gir26:Share><gir26:RateTopUp currency="USD">${amount(r.rateTopUp)}</gir26:RateTopUp><gir26:ACTTT currency="USD">${amount(r.acttt)}</gir26:ACTTT><gir26:AllocatedTopUp currency="USD">${amount(r.topUp)}</gir26:AllocatedTopUp></gir26:CE>`).join("\n")}
      </gir26:CEDetail>
